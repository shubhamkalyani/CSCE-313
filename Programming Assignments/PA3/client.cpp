...
void file_thread_function(string file_name, BoundedBuffer* requestBuffer, FIFORequestChannel* channel, int buffer_size) {
...
void worker_thread_function(BoundedBuffer* reqBuffer, FIFORequestChannel* channel, BoundedBuffer* respBuffer, int bufferSize) {
...
int main(int argc, char *argv[]) {
    ...
    vector<FIFORequestChannel*> wchans(w);

    for (int i = 0; i < w; i++) {
        MESSAGE_TYPE m = NEWCHANNEL_MSG;
        chan->cwrite(&m, sizeof(m));
        char newchan[1024];
        chan->cread(newchan, sizeof(newchan));
        wchans[i] = new FIFORequestChannel(newchan, FIFORequestChannel::CLIENT_SIDE);
    }
